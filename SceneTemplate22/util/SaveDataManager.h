...
private:
	...
	{
		int clearTime; // クリア時間
		std::unordered_map<e_PlayerKind, bool> isRelease; // プレイヤーの種類ごとの解放状態
...
public:
	...
	// プレイヤーの種類を解放する関数
	void OnRelease(e_PlayerKind kind) { m_info.isRelease[kind] = true; }

	// クリア時間を取得する関数
	int GetClearTime() const { return m_info.clearTime; }
	// プレイヤーの種類が解放されているかを確認する関数
	bool IsRelease(e_PlayerKind kind) const { return m_info.isRelease.at(kind); }
