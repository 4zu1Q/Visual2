...
//	MV1_COLL_RESULT_POLY* Kabe[2048];	// 壁ポリゴンと判断されたポリゴンの構造体のアドレスを保存しておくためのポインタ配列
//	MV1_COLL_RESULT_POLY* Yuka[2048];	// 床ポリゴンと判断されたポリゴンの構造体のアドレスを保存しておくためのポインタ配列
...
//
//	// 移動後の座標を算出
//	NowPos = VAdd(playerPosition, MoveVector);
//
//	// プレイヤーの周囲にあるステージポリゴンを取得する
//	// ( 検出する範囲は移動距離も考慮する )
//	HitDim = MV1CollCheck_Sphere(stageModelHandle, -1, playerPosition, 800.0f + VSize(MoveVector));
//
...
//				if (HitDim.Dim[i].Position[0].y > playerPosition.y + 1.0f ||
//					HitDim.Dim[i].Position[1].y > playerPosition.y + 1.0f ||
//					HitDim.Dim[i].Position[2].y > playerPosition.y + 1.0f)
//				{
...
//
//				// ポリゴンとプレイヤーが当たっていなかったら次のカウントへ
//				if (HitCheck_Capsule_Triangle(NowPos, VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), 200.0f, Poly->Position[0], Poly->Position[1], Poly->Position[2]) == FALSE) continue;
//
//				// ここにきたらポリゴンとプレイヤーが当たっているということなので、ポリゴンに当たったフラグを立てる
//				HitFlag = 1;
//
...
//
//					// 当たっていたらループから抜ける
//					if (HitCheck_Capsule_Triangle(NowPos, VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), 200.0f, Poly->Position[0], Poly->Position[1], Poly->Position[2]) == TRUE) break;
//				}
//
...
//
//				// ポリゴンに当たっていたら当たったフラグを立てた上でループから抜ける
//				if (HitCheck_Capsule_Triangle(NowPos, VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), 200.0f, Poly->Position[0], Poly->Position[1], Poly->Position[2]) == TRUE)
//				{
//					HitFlag = 1;
//					break;
//				}
...
//				LineRes = HitCheck_Line_Triangle(NowPos, VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), Poly->Position[0], Poly->Position[1], Poly->Position[2]);
//
//				// 接触していなかったら何もしない
...
//					LineRes = HitCheck_Line_Triangle(VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), VAdd(NowPos, VGet(0.0f, -1.0f, 0.0f)), Poly->Position[0], Poly->Position[1], Poly->Position[2]);
//				}
//				else
//				{
//					// 走っている場合は頭の先からそこそこ低い位置の間で当たっているかを判定( 傾斜で落下状態に移行してしまわない為 )
//					LineRes = HitCheck_Line_Triangle(VAdd(NowPos, VGet(0.0f, 700.0f, 0.0f)), VAdd(NowPos, VGet(0.0f, -40.0f, 0.0f)), Poly->Position[0], Poly->Position[1], Poly->Position[2]);
//				}
//
//				// 当たっていなかったら何もしない
//				if (LineRes.HitFlag == FALSE) continue;
